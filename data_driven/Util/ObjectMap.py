...
# 查找一个元素的封装
def find_element(driver,loacte_method,locate_exp):
    try:
        # 设置超时时间，超时后报异常
        element=WebDriverWait(driver,10).until(lambda x:x.find_element(loacte_method,locate_exp))
...
# 查找多个元素的封装
def find_elements(driver, loacte_method, locate_exp):
    try:
        elements = WebDriverWait(driver, 10).until(lambda x: x.find_elements(loacte_method, locate_exp))
